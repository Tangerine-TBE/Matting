...
class AgreementContentActivity:AppCompatActivity() {
    companion object{
        ...
        const val email = "2681706890@qq.com"
        const val com = "深圳市天王星互娱科技有限公司"
    }
    ...
    private fun getFuwu():SpannableStringBuilder{
        ...
        um.setSpan(object : ClickableSpan() {
            override fun onClick(view: View) {
                ...
                intent.putExtra(WebViewActivity.URL_KEY, umUrl)
        ...
        gdt.setSpan(object : ClickableSpan() {
            override fun onClick(view: View) {
                val intent = Intent(this@AgreementContentActivity, WebViewActivity::class.java)
                intent.putExtra(WebViewActivity.URL_KEY, gdtUrl)
                startActivity(intent)
            }
        ...
        tt.setSpan(object : ClickableSpan() {
            override fun onClick(view: View) {
                val intent = Intent(this@AgreementContentActivity, WebViewActivity::class.java)
                intent.putExtra(WebViewActivity.URL_KEY, ttUrl)
                startActivity(intent)
            }
        }, 0, tt.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
        weChat.setSpan(object : ClickableSpan() {
            override fun onClick(view: View) {
                val intent = Intent(this@AgreementContentActivity, WebViewActivity::class.java)
                intent.putExtra(WebViewActivity.URL_KEY, weChatUrl)
                startActivity(intent)
            }
        }, 0, weChat.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)

        qq.setSpan(object : ClickableSpan() {
            override fun onClick(view: View) {
                val intent = Intent(this@AgreementContentActivity, WebViewActivity::class.java)
                intent.putExtra(WebViewActivity.URL_KEY, qqUrl)
                startActivity(intent)
            }
        }, 0, qq.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
        ...
        stringBuilder.append("隐私政策\n" +
                ...
                "2.访问手机存储设备\n" +
                "\n" +
                "${packageManager.getApplicationLabel(applicationInfo)}需要访问存储设备以存储文件\n" +
                "\n" +
                "3.相机权限\n" +
                "\n" +
            ...
            .append("\n" +
                    ...
                    "\n" +
                    "1、由于您将用户密码告知他人或与他人共享注册帐户，由此导致的任何个人信息的泄露，或其他非因${packageManager.getApplicationLabel(
                        applicationInfo
                    )}原因导致的个人信息的泄露； \n" +
                    "\n" +
