...
class CropActivity : BaseActivity(), CropImageView.OnCropImageCompleteListener {
    ...
    override fun initView() {
        ...
        ll_crop.setOnClickListener {
            tv_crop.setTextColor(Color.parseColor("#FFD23D"))
            tv_rotate.setTextColor(Color.parseColor("#66ffffff"))
            visible(view_crop, crop_root_view)
            invisible(rotate_root_view)
        }

        ll_rotate.setOnClickListener {
            tv_rotate.setTextColor(Color.parseColor("#FFD23D"))
            tv_crop.setTextColor(Color.parseColor("#66ffffff"))
    ...
    override fun onCropImageComplete(view: CropImageView?, result: CropImageView.CropResult?) {
        if (cachePath != "" && cacheFileName != ""){
            ...
            if (!rootFile.exists())
                rootFile.mkdirs()
            Thread{
                BitmapUtils.saveImageToGallery(result?.bitmap)
                runOnUiThread {
                    loadingDialog.dismiss()
